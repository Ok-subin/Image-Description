...
void MeanShift::colorRecognition(int R, int G, int B, string resultArray[3][100], int imageIndex)
{
	float H, S, V;
	...
	if (S < 0.27)
	{
		...
		resultArray[1][imageIndex] = color;
	}
		...
		resultArray[1][imageIndex] = colorName[minIndex];
	}
}

void MeanShift::mainExe(string bbImageName, string saveSegmentImage, string saveColorImage, string resultArray[3][100], int imageIndex)
{
	...
	colorRecognition(rValue, gValue, bValue, resultArray, imageIndex);
}